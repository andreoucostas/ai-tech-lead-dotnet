...
`.claude/commands/docs-sync.md` is the single source of truth. Follow it exactly: check `CLAUDE.md` against the codebase → check `copilot-instructions.md` against `CLAUDE.md` → check "What We've Learned" → check `TECH_DEBT.md` against the codebase → present a structured drift report.

...
> Status note: the VS Code `permissionDecision` path is implemented in `guard.*` and verified by the Task 0 spike (`scratchpad/task0-vscode-hook-spike`). If your environment cannot enable Preview hooks, the VS Code column degrades to "instructed only" — by design, not by omission.
